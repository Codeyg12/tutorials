  ...
  >
    <div class="flex flex-col gap-2">
      <p class="opacity-60 text-base sm:text-lg md:text-xl text-center">
        For all your questions, qualms, and quandaries, we've got you covered.
      </p>
      <h3
        class="text-4xl sm:text-5xl md:text-6xl mx-auto w-full font-semibold text-center max-w-[1000px]"
      >
    ...
    <div class="flex flex-col gap-8 sm:gap-10 md:gap-14 w-full">
      {#each faqs as faq}
        <div
          class="flex flex-col gap-2 text-left w-full mx-auto relative max-w-[800px] p-4 px-6"
        >
          <div
            class="absolute top-0 left-0 bg-slate-950 -translate-x-4 w-1/3 h-[1px]"
          ></div>
          <div
            class="absolute top-0 left-0 bg-slate-950 -translate-y-4 h-2/3 w-[1px]"
          ></div>
